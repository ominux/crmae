...
In order to leverage the volatile STT-RAM properties at the last level cache hierarchy, we need to
...
an ideal retention time by analyzing the retention times of last level cache blocks in a
multithreaded PARSEC~\cite{bienia11benchmarking} and multiprogrammed SPEC 2006~\cite{spec}
environment. The idea is to understand the distribution of the inter-write intervals to a last level
cache and use average of these intervals as the STT-RAM retention time. The next sub-section
...
L2 cache block. We define this interval to be {\it revival time}. While collecting these results, we
...
obtained by running multithreaded (PARSEC~\cite{PARSEC}) and multiprogrammed (SPEC 2006~\cite{spec})
...
Figures~\ref{fig:distribution} (a) and (b) show the results of three PARSEC and SPEC benchmarks along
with the averages across 12 PARSEC  and 14 SPEC benchmarks, respectively. We observe from the figures
that, on an average, approximately 50\% of the cache blocks get refreshed within 10{\it ms}, this is
in contrast to the microsecond reuse for L1 case~\cite{3t1d-cache}. About 20\% of blocks remain in
the cache for more than 40{\it ms} and rest of the blocks have intermediate revival times. Blocks
...
would affect the write latency.
...
Note that one can possibly lower the retention time further beyond the {\it ms} ranges, but it becomes much harder to control the variations and the benefits of performance/energy diminish. For the sake of correctness and preciseness, we discuss these designs only in the paper.
