...
indicates that it is going to expire in time T. We define this time as the {\it leftover time} and
...
granularity. For example, if we use a 2-bit counter, the {\it leftover time} is 2.5{\it ms} and for a
3-bit counter it is 1.25{\it ms}. A large bit counter decreases the {\it leftover time} and allows
...
steps of 2.5{\it ms} and at any time the block is written/invalidated, it goes back to the initial
...
buffer. \ding{183} checks the occupancy of the buffer and if it is not full, each of the diminishing
...
they are dirty, we write back those blocks as shown in \textcircled{\raisebox{-.9pt}{C}}.  If they
are not dirty, they are invalidated.
...
\centering
 \psfig{figure=figures/cdf.eps, width=3in, height=1.9in}
  \caption{\label{fig:cdf} Cumulative Distribution of Dead Blocks per Bank with number of ways.}
%\end{figure*}
\end{figure*}
