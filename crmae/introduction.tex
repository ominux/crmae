...
%b) [Application Level] Our exhaustive application-level analysis with 25 benchmarks from SPEC and PARSEC suites (compared to only 5 applications in [19]) shows that μs retention time proposed in [19], is not suitable for last level cache because of too much refresh overhead.
 
%c) [Architecture Level] We feel that analyzing this broad spectrum of applications and architecting the whole cache hierarchy is a non-trivial task. Our refresh scheme is simple with less area/power/performance overhead compared to costly DRAM style refreshing.%
